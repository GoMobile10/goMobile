...
/**
 * The class describes the list with the components which have to be picked up
 * to repair the bike. This list is connected to the database and different for
 * each bike. Each component is marked with a red icon to show the component
 * isn't already picked up. After scanning the component the icon switches to a
 * green "Check".
 * 
 * @author Patrick
 * 
...
public class Pickuplist extends ViewWithNavigation {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ListAdapter listenAdapter = new PickupSparePartsListAdapter(
				Pickuplist.this, R.layout.rowlayout_sparepartspickuplist_item,
				needed_sparparts);
		meineListView = (ListView) findViewById(R.id.listView1);
		meineListView.setAdapter(listenAdapter);

		setContentView(createNavigationInfo(R.id.pickuplistId, this, "back",
				"detail", "help", "save"));

	}
/**
 * Check if the component is already picked up or not
 */
	private void checkScannedSpareParts() {
		...
		if (intent.getStringExtra("scannedEAN") != null) {
			for (int i = 0; i < needed_sparparts.size(); i++) {
				if (needed_sparparts.get(i).getEanNumber() == Long.parseLong(
						intent.getStringExtra("scannedEAN"), 10)) {
					needed_sparparts.get(i).setPicukuped(true);
					Toast.makeText(
							getApplicationContext(),
							"You picked up: "
									+ needed_sparparts.get(i).getDescription()
									+ " ("
									+ needed_sparparts.get(i).getEanNumber()
		...
		if (intent.getStringExtra("scannedEAN") != null) {
			for (int h = 0; h < scannedComponents.size(); h++) {
				for (int i = 0; i < needed_sparparts.size(); i++) {
					if (needed_sparparts.get(i).getEanNumber() == Long
							.parseLong(scannedComponents.get(h), 10)) {
						needed_sparparts.get(i).setPicukuped(true);
					}
				}
			}
	...
	private void populateSpareParts() {
		...
		repairorderlist = bikeDataController.getRepairOrders("BikeEAN="
				+ bikeEAN);
		...
		textView.setText("Bike: "
				+ getIntent().getExtras().getString("BikeName"));
		scannedBefore = true;
		try {
			scannedComponents = getIntent().getStringArrayListExtra(
					"scannedComponents");
		} catch (Exception e) {
		...
		HashMap<Component, Component> defectcomponents = (HashMap<Component, Component>) tempRepairOrder
				.getDefectReplacementComponentMap();
	...
	public void navigateUp() {
		...
		repairTodoList.putExtra("BikeName",
				getIntent().getExtras().getString("BikeName"));
